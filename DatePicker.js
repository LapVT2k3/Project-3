...
class DatePicker {
	...
	renderCalendar(tbl) {
		...
		function handleCellClick(id, x, firstDayofMonth) {
			self.callback(id, { day: x - firstDayofMonth + 1, month: curMonth, year: curYear });
		}
		...
		while (i < firstDayofMonth + lastDateOfMonth + 6 - lastDayofMonth) {
			...
			td.id = 'C' + i;
			const isToday = i - firstDayofMonth + 1 === timeNow.getDate() 
					&& curMonth === timeNow.getMonth()
					&& curYear === timeNow.getFullYear();
			if (i < firstDayofMonth) {
				td.appendChild(document.createTextNode(lastDateOfLastMonth - firstDayofMonth + i + 1));
				td.style.color = '#aaa';
				...
				td.appendChild(document.createTextNode(i - lastDateOfMonth - firstDayofMonth + 1));
				td.style.color = '#aaa';
			}
